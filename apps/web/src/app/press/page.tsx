    ...
    <div className="min-h-screen bg-naploo-dark text-white">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-cyan-500/10 rounded-full blur-3xl" />
      ...
      <section className="relative py-12 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-center text-white/50 mb-8">Featured In</p>
          <div className="flex flex-wrap justify-center gap-8">
            {mediaFeatures.map((media) => (
              <div key={media.name} className="flex items-center gap-2 text-white/40 hover:text-white/60 transition-colors">
          ...
          <div className="space-y-6">
            {pressReleases.map((release, index) => (
              <Link
                key={index}
                href={release.link}
                className="block glass-card rounded-xl p-6 hover:ring-2 hover:ring-primary-500/50 transition-all group"
              >
                <div className="flex items-start gap-4">
                  ...
                  <ExternalLink className="w-5 h-5 text-white/30 group-hover:text-primary-400 flex-shrink-0" />
                </div>
              </Link>
            ))}
          </div>
      ...
      <section className="relative py-24 bg-white/[0.02]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-12">
            <div>
              <h2 className="text-3xl font-bold text-white mb-2">Brand Assets</h2>
              <p className="text-white/60">Download logos, images, and brand guidelines</p>
            </div>
            <button className="hidden sm:flex items-center gap-2 px-4 py-2 bg-white/10 rounded-lg text-white hover:bg-white/20 transition-colors">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/50">{asset.type} • {asset.size}</p>
                  </div>
                </div>
                <button className="p-2 text-white/50 hover:text-primary-400 transition-colors">
